...
namespace BicUtil.Tween{
	public class Bezier {
		...
		public Bezier(Vector3 a, Vector3 b, Vector3 c, Vector3 d, float precision){
			this.a = a;
			aa = (-a + 3*(b-c) + d);
			bb = 3*(a+c) - 6*b;
			cc = 3*(b-a);
	...
	public class BezierPath {
		...
		public BezierPath(){ }
		public BezierPath( Vector3[] pts_ ){
			setPoints( pts_ );
		}

		public void setPoints( Vector3[] pts_ ){
			if(pts_.Length<4)
				Debug.LogError( "[BicTween] - When passing values for a vector path, you must pass four or more values!" );
			if(pts_.Length != 4 && pts_.Length % 3 != 1)
				Debug.LogError( "[BicTween] - When passing values for a vector path, they must be in sets of four: controlPoint1, controlPoint2, endPoint2, controlPoint2, controlPoint2..." );

			pts = pts_;

			int k = 0;
			beziers = new Bezier[ (pts.Length - 1) / 3 ];
			lengthRatio = new float[ beziers.Length ];
			int i;
			length = 0;
			for(i = 0; i < pts.Length-3; i+=3){
				beziers[k] = new Bezier(pts[i+0],pts[i+1],pts[i+2],pts[i+3],0.05f);
