...
// public static class LeanTweenOnEditor {
...
// 	public static List<Action<int, float>> UpdateCallbacks = new List<Action<int, float>>();
// 	public static int callbackId = 0;
// 	public static void AddUpdateCallback(Action<int, float> _callback){
// 		UpdateCallbacks.Add(_callback);
// 	}

// 	public static void RemoveUpdateCallback(int _hashCode){
// 		for(int i = 0; i < UpdateCallbacks.Count; i++){
// 			if(UpdateCallbacks[i].GetHashCode() == _hashCode){
...
// 	public static void ClearUpdateCallback(){
...
// 		if(/*frameRendered != Time.frameCount*/ true){ // make sure update is only called once per frame
// 			init();

// 			//#if UNITY_3_5 || UNITY_4_0 || UNITY_4_0_1 || UNITY_4_1 || UNITY_4_2 || UNITY_4_3 || UNITY_4_5
// 			LeanTween.dtEstimated = Time.realtimeSinceStartup - previousRealTime;
// 			if(LeanTween.dtEstimated>0.2f) // a catch put in, when at the start sometimes this number can grow unrealistically large
// 			LeanTween.dtEstimated = 0.2f;
// 			previousRealTime = Time.realtimeSinceStartup;
// 			//#else

// 			//LeanTween.dtEstimated = LeanTween.dtEstimated<0f ? 0f : LeanTween.dtEstimated = 1f/60f;
// 			//Debug.Log("dt = " + LeanTween.dtEstimated.ToString());
// 			//#endif
// 			for(int i = 0; i < UpdateCallbacks.Count; i++){
// 				UpdateCallbacks[i](UpdateCallbacks[i].GetHashCode(), LeanTween.dtEstimated);
// 			}
			
// 			dtActual = 1/60f; //Time.deltaTime;
// 			maxTweenReached = 0;
// 			finishedCnt = 0;
...
// 			frameRendered++; //frameRendered = Time.frameCount;
...
// 						MonoBehaviour.Destroy(tweens[i].trans.gameObject);
// 					}
// 				}
// 			}
